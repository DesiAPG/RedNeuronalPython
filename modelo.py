...
class Linear_QNet(nn.Module):
    def __init__(self, tamaño_entrada, tamaño_oculto, tamaño_salida):
        super().__init__()
        self.linear1 = nn.Linear(tamaño_entrada, tamaño_oculto)
        self.linear2 = nn.Linear(tamaño_oculto, tamaño_salida)
...
class QTrainer:
    ...
    def train_step(self, state, action, reward, next_state, done):
        ...
        # 1: Predecir los valores de Q con el estado actual
        ...
        for idx in range(len(done)):
            ...
            if not done[idx]:
                Q_new = reward[idx] + self.gamma * \
                    torch.max(self.modelo(next_state[idx]))
